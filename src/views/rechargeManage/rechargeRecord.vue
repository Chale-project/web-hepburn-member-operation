  ...
  <div style="background:#fff ;padding:25px;position: relative">
        ...
        <a-row :gutter="48">
          ...
          <a-col :md="8" :sm="24">
            <a-form-item label="会员昵称">
              <a-input v-model="queryParam.buyerNick" placeholder="请输入充值会员昵称" />
            </a-form-item>
          </a-col>
            ...
            <a-col :md="8" :sm="24">
              <a-form-item label="订单来源">
                <a-select placeholder="请选择" v-model="queryParam.orderSource">
                  ...
                  <a-select-option value="rechargeOpp">运营平台代充</a-select-option>
                  <a-select-option value="rechargeMini">小程序直充</a-select-option>
                  <a-select-option value="rechargePresent">充值赠送</a-select-option>
    ...
    <a-table size="middle" :columns="columns" :dataSource="loadDatas" :loading="loading" :pagination="pagination" :rowSelection="{type:'checkbox', selectedRowKeys: selectedRowKeys, onChange: updateSelect }">
      ...
      <template slot="orderSource" slot-scope="record">
        {{record=='rechargeOpp'?'运营平台代充':record=='rechargeMini'?'小程序直充':record=='rechargePresent'?'充值赠送':record=='reward'?'打赏':'其他'}}
      </template>
...
export default {
  ...
  data() {
    return {
      ...
      queryParam: {
        // 搜索查询参数
        orderType: 'recharge', // recharge：消费；consume：消费
        shopId: '',
        buyerNick: null,
        userNumber: null,
        orderSource: '',
  ...
  methods: {
    ...
    // 操作充值记录
    handleRechargeRecord(e, o) {
      this.rechargeRecordInfo = {}
      this.editType = e
      if (e == 'detail') {
        this.modalTitle = '充值记录详情'
        this.getRechargeRecordInfo(o.id)
      }
    },
    ...
    // 获取充值记录信息
    getRechargeRecordInfo(id) {
      let _id = null
      if (id) _id = id
      else _id = this.selectedRows[0].id
      getRechargeRecordInfo(_id)
        .then(res => {
          if (res.code == 0) {
            this.visible = !0
            const _info = res.result
            this.rechargeRecordInfo = _info
          } else {
            this.$message.error(res.msg)
          }
        })
